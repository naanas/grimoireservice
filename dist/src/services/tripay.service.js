...
const getTripayConfig = (overrideMode, overrideKey, overridePriv, overrideCode) => {
 ...
 */
export const initPayment = async (trxId, amount, buyerName, buyerEmail, buyerPhone, productName, paymentChannel, basePrice, adminFee, 
// Optional Config Overrides
optMode, optApiKey, optPrivateKey, optMerchantCode) => {
    ...
    payloadObj.signature = crypto.createHmac('sha256', conf.privateKey).update(signatureStr).digest('hex');
    try {
        ...
        if (resData.success) {
            ...
            let paymentNo = null;
            if (payloadData.qr_string)
                paymentNo = payloadData.qr_string;
            else if (payloadData.qr_url)
                paymentNo = payloadData.qr_url;
            else
                paymentNo = payloadData.pay_code;
            return {
                success: true,
                message: "Success",
