...
const mapChannelToDupayMethod = (paymentMethod, channel) => {
    ...
    if (directMap[normalizedChannel])
        return directMap[normalizedChannel];
    if (directMap[method])
        return directMap[method];
    if (method === 'va')
        return 'BCAVA';
    if (method === 'ewallet')
        return 'DANA';
    if (method === 'retail' || method === 'cstore')
        return 'ALFAMART';
    return 'QRISC';
};
export const initPayment = async (trxId, amount, paymentMethod, paymentChannel, cfg) => {
...
//# sourceMappingURL=dupay.service.js.map