 ...
 */
export const processGameTopup = async (trxId) => {
    ...
    try {
        ...
        const order = await gameProvider.placeOrder(trx.invoice, trx.product.sku_code, trx.targetId || '', trx.zoneId || '');
...
export const getCategories = async (req, res) => {
    try {
        ...
        for (const cat of categories) {
            const brand = cat.brand || cat.name; // Fallback to name if brand not set
...
export const checkGameId = async (req, res) => {
    const { gameCode, gameSlug, userId, zoneId } = req.body;
...
const normalizeClientIp = (ip) => ip.replace(/^::ffff:/, '').trim();
/** Client IP for webhook auth — uses req.ip when trust proxy is enabled (Render). */
const getCallbackClientIp = (req) => normalizeClientIp(req.ip || req.socket.remoteAddress || '');
