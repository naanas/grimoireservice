...
const processGameTopup = async (trxId) => {
    ...
    try {
        ...
        const order = await apigamesService.placeOrder(trx.invoice, trx.product.sku_code, trx.targetId || '', // Player ID
        trx.zoneId || '' // Zone ID
        );
        if (order.success && order.data) {
            ...
            if (order.data.status.toLowerCase() === 'sukses')
                newStatus = 'SUCCESS';
            await prisma.transaction.update({
                where: { id: trxId },
                data: {
...
export const createTransaction = async (req, res) => {
    ...
    try {
        ...
        if (!product)
            return res.status(404).json({ success: false, message: 'Product Not Found (Mock ID mismatch or DB Error)' });
        // 2. Validate Game ID (Optional Check)
        let gameCode = 'mobilelegend';
        if (product.category?.slug === 'free-fire')
            gameCode = 'freefire';
        if (product.category?.slug === 'mobile-legends')
        ...
        if (!profile.success)
            return res.status(400).json({ success: false, message: 'Invalid Game ID/Server' });
        ...
        if (paymentMethod === 'BALANCE') {
            ...
            if (!authHeader)
                return res.status(401).json({ success: false, message: 'Authentication required for Balance payment' });
            ...
            let payerId;
            try {
                const decoded = jwt.verify(token, process.env.JWT_SECRET || 'super-secret-key-change-this');
                payerId = decoded.id;
            }
            catch (err) {
...
// POST /api/callback/ipaymu
// POST /api/callback/ipaymu
export const handleIpaymuCallback = async (req, res) => {
    try {
...
export const handleApigamesWebhook = async (req, res) => {
    try {
