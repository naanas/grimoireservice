...
export const CHAT_AUTO_REPLY =
    'Mohon tunggu sebentar, pesan Anda sedang kami teruskan ke admin. Tim kami akan membalas sesegera mungkin.';

const waNotifyCooldown = new Map<string, number>();
...
export async function sendChatAutoReplyIfNeeded(sessionId: string, io: Server, adminOnline: boolean) {
    if (adminOnline) return null;
...
export async function notifyAdminChatActivity(
    sessionId: string,
    options: { customerLabel: string; preview?: string },
) {
    const now = Date.now();
    const last = waNotifyCooldown.get(sessionId) ?? 0;
    if (now - last < WA_COOLDOWN_MS) return;
...
export async function getSessionCustomerLabel(sessionId: string) {
    ...
    if (session.guestEmail) return session.guestEmail;
    return 'Guest';
}
