...
export const checkTransaction = async (refId: string, providerId?: string) => {
    // Apigames uses RefID (our invoice)
    // VIP uses Provider ID (trxid)

    if (PROVIDER === 'VIP') {
        if (!providerId) return { success: false, message: 'VIP requires Provider ID for status check' };
